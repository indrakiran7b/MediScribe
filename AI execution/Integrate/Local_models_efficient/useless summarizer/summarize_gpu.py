...
class MedicalTranscriptSummarizer:
    def __init__(self, model_name: str = "facebook/bart-large-cnn"):
        ...
        # Ensure the model is loaded correctly with the device map
        ...
        # Move the model to the correct device using to_empty()
        self.model = self.model.to_empty(self.device)
    ...
    def generate_summary_prompt(self, text: str) -> str:
        """
        Generate a prompt that guides the model to produce a medical summary.
        """
        return f"""Summarize this medical consultation, focusing on key medical information:

{text}

Key points to include:
1. Patient's main symptoms and concerns
2. Relevant medical history
3. Doctor's observations and potential diagnosis
4. Any medications discussed
5. Recommendations and follow-up plans"""

    def summarize(self, file_path: str) -> Dict:
        """
        Process JSON transcript and generate a comprehensive medical summary.
        """
        ...
        patient_friendly = self.generate_patient_friendly_summary({
        ...
        return {
            "full_summary": final_summary,
            "patient_friendly_summary": patient_friendly,
            "key_points": key_points,
            "task": task,
            "timestamp": timestamp
        }
...
def summarize_transcript(file_path):
    ...
    output += result["patient_friendly_summary"] + "\n\n"
    output += "Key Points:\n"
